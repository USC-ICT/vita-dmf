...
public class CutsceneIO
{
    ...
    public void SaveMachinimaData(List<TimelineObject> cutscenes, string path)
    {
        ...
        {
            Cutscene c = cutscenes[cutsceneIndex] as Cutscene;
            m_SaveData.CutsceneDatas.Add(c.m_CutsceneData);

            for (int eventIndex = 0; eventIndex < c.CutsceneEvents.Count; eventIndex++)
            {
                CutsceneEvent ce = c.CutsceneEvents[eventIndex];
                CutsceneEvent savedCe = m_SaveData.CutsceneDatas[cutsceneIndex].Events[eventIndex];

                // I have to save this information manually post unity 4.3 so that I can reload events
                savedCe.TargetComponentName = ce.TargetComponent.GetType().ToString();

                // same thing here
                for (int paramIndex = 0; paramIndex < ce.m_Params.Count; paramIndex++)
                {
                    CutsceneEventParam cep = ce.m_Params[paramIndex];
                    CutsceneEventParam savedCep = savedCe.m_Params[paramIndex];
                    savedCep.objDataName = cep.objDataName;
                    savedCep.objDataIsComponent = cep.objDataIsComponent;
                    savedCep.usesObjectData = cep.usesObjectData;
                    savedCep.objDataInstanceId = cep.objDataInstanceId;
                }
            }

            //m_SaveData.CutsceneDatas[i].TrackGroups.AddRange(c.GroupManager.m_TrackGroups);
            TrackGroups.Add(new List<CutsceneTrackGroup>());
            TrackGroups[cutsceneIndex].AddRange(c.GroupManager.m_TrackGroups);
        }

        VHFile.WriteXML<MachinimaSaveData>(path, m_SaveData);
    }
    ...
    public void SaveCutscene(string path, Cutscene cutscene)
    {
        ...
        VHFile.WriteXML<CutsceneData>(path, cutscene.m_CutsceneData);
    }
    ...
    public void LoadMachinimaData(List<TimelineObject> cutscenes)
    {
        ...
        //Debug.Log("allGameObjects. count: " + allGameObjects.Length);
        ...
        for (int i = 0; i < m_SaveData.CutsceneDatas.Count; i++)
        {
            ...
            if (cutscene != null)
            {
                ...
                for (int j = 0; j < cutscene.m_CutsceneData.Events.Count; j++)
                {
                    ...
                    CutsceneEvent savedCe = m_SaveData.CutsceneDatas[i].Events[j];

                    // something broke in unity 4.3 where the component doesn't get saved, so I have to use the component name to try and find it
                    ce.SetFunctionTargets(savedCe.TargetGameObject, savedCe.TargetGameObject.GetComponent(savedCe.TargetComponentName));

                    // this was added as well post 4.3
                    for (int k = 0; k < ce.m_Params.Count; k++)
                    {
                        CutsceneEventParam cep = ce.m_Params[k];
                        CutsceneEventParam savedCep = savedCe.m_Params[k];
                        if (savedCep.objDataIsComponent && savedCep.objDataInstanceId != 0)
                        {
                            GameObject[] goData = Array.FindAll<GameObject>(allGameObjects, go => (go.GetInstanceID() == cep.objDataInstanceId));
    ...
    {
        CutsceneData csData = VHFile.ReadXML<CutsceneData>(path);
    ...
    void SetupEventObjects(Cutscene cutscene, CutsceneEvent ce, GameObject[] allGameObjects)
    {
        if (!GenericEventNames.IsCustomEvent(ce.EventType))
        {
            ...
            {
                Debug.LogError("Couldn't find the GenericEvent component associated with event type: " + ce.EventType);
                return;
            }
            ce.TargetComponent = genericEventsComp.GetGenericEventsByEventType(ce.EventType);
