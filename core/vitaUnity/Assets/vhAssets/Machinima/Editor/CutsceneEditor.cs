...
public class CutsceneEditor : TimelineWindow
{
    ...
    const string LastFolderPathKey = "LastMMFolderPath";
    ...
    public class SequencerData
    {
        ...
        public List<string> m_QueuedFilesToLoad = new List<string>();
    ...
    public bool CreateBMLEvents
    {
        get { return m_CreateBMLEvents; }
        set { m_CreateBMLEvents = value; }
    }
    #endregion
...
#else
        window.titleContent.text = "Machinima";
#endif
    ...
    protected override void Update()
    {
        if (m_CleanRequestState != CleanRequest.None)
        {
            ...
            CleanRequest prevRequest = m_CleanRequestState;

            // clear it now because if something goes wrong in the loading, we start to loop adding cutscenes to the scene
        ...
        if (m_SequencerData.m_QueuedFilesToLoad.Count > 0)
        {
            int indexToUse = m_SequencerData.m_QueuedFilesToLoad.Count - 1;
            RequestFileOpenBMLXMLPair(m_SequencerData.m_QueuedFilesToLoad[indexToUse].Replace("\\", "/"));
            m_SequencerData.m_QueuedFilesToLoad.RemoveAt(indexToUse);
        }
    ...
    {
        string sceneName = Application.isPlaying ? VHUtils.SceneManagerActiveSceneName() : VHUtils.EditorSceneManagerActiveSceneName();
    ...
    void DrawCutscene()
    {
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            GUILayout.BeginVertical();
            {
                m_SelectedDataViewTab = (DataView)GUILayout.Toolbar((int)m_SelectedDataViewTab, DataViewNames, GUILayout.Width(m_EventDataColumnWidth));
                ...
                GUILayout.FlexibleSpace();
                m_EventDataColumnWidth = EditorGUILayout.Slider(m_EventDataColumnWidth, EventDataWidth_MAX, EventDataWidth_MIN);
            }
            GUILayout.EndVertical();
        }
        GUILayout.EndHorizontal();
    ...
    bool DrawCutsceneTrackItem(CutsceneTrackItem trackItem, float timelineStartTime, float timelineEndTime, Rect scrollArea)
    {
        ...
        if (VHMath.IsRectOverlapping(testArea, trackItem.GuiPosition))
        {
    ...
    protected override void DrawEventData(List<CutsceneTrackItem> selectedEvents)
    {
        ...
        m_CutsceneEventDataScrollPos = GUILayout.BeginScrollView(m_CutsceneEventDataScrollPos);
        {
            ...
            if (m_ShowNotes)
            {
                selectedEvent.Notes = GUILayout.TextArea(selectedEvent.Notes, GUILayout.Height(NoteBoxHeight), GUILayout.MaxWidth(m_EventDataColumnWidth));
            }
    ...
    void ChangedEventGameObjectSelection(GameObject newSelection, CutsceneEvent selectedEvent, Component selectedComponent)
    {
        ...
        if (selectedComponent == null)
        {
            ...
            selectedEvent.SetFunctionTargets(selectedEvent.TargetGameObject, selectedEvent.TargetGameObject.GetComponent(m_EventComponentNames[m_SelectedEventComponent]));
        }
    ...
    void DrawEventFunctionParameters(CutsceneEvent cutsceneEvent)
    {
        ...
        ParameterInfo[] selectedFunctionParams = cutsceneEvent.GetFunctionParams();
        ...
        for (int i = 0; i < selectedFunctionParams.Length; i++)
        {
            ...
            else if (functionParam.ParameterType == typeof(string))
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    {
                        StringEditPopup.Init(new Rect(position.x + position.width - m_EventDataColumnWidth, position.y, m_EventDataColumnWidth, 250), cutsceneParam.Name, cutsceneParam.stringData, OnSavedLongString);
                    }
                }
                EditorGUILayout.EndHorizontal();
            ...
            else if (functionParam.ParameterType.GetType().IsInstanceOfType(typeof(UnityEngine.Object))) // this needs to go last
            {
                ...
                cutsceneParam.SetObjData(EditorGUILayout.ObjectField(cutsceneParam.Name, cutsceneParam.objData, functionParam.ParameterType, true));
    ...
    void GatherGameObjectComponentNames(GameObject target)
    {
        ...
        List<MonoBehaviour> holder = new List<MonoBehaviour>();
        for (int i = 0; i < attachedBehaviours.Length; i++)
        {
            if (attachedBehaviours[i] != null)
            {
                holder.Add(attachedBehaviours[i]);
            }
            else
            {
                GameObject.DestroyImmediate(attachedBehaviours[i]);
            }
        }

        attachedBehaviours = holder.ToArray();
    ...
    void GatherComponentFunctionNames(Component behaviour, CutsceneEvent ce, bool resetData)
    {
        if (behaviour == null)
        {
            Debug.Log("GatherComponentFunctionNames failed. behaviour is null");
            return;
        }

        if (ce == null)
        {
            Debug.Log("GatherComponentFunctionNames failed. event is null");
            return;
        }
    ...
    void PlayCutscene(Cutscene cutscene, bool play)
    {
        ...
        if (Application.isPlaying)
        {
            ...
            {
                // this was set to 0 before, which pauses the entire scene.
                // if this causes problems, set it back to 0
                Time.timeScale = 1;
                cutscene.Pause();
            }
        }
    ...
    void DuplicateCutsceneMenuItem(object cutsceneToDuplicate)
    {
        DuplicateCutscene((Cutscene)cutsceneToDuplicate);
    }

    public void DuplicateCutscene(Cutscene cutsceneToDuplicate)
    {
        Cutscene duplicate = GameObject.Instantiate<Cutscene>(cutsceneToDuplicate);
        duplicate.CutsceneName = cutsceneToDuplicate.name + " (Clone)";
        ChangedData();
        LoadAllTimelineObjects<Cutscene>();
        m_SelectedIndex = m_TimeObjectManager.GetTimeObjectIndexByName(duplicate.name);
        CutsceneSelected(m_SelectedIndex);
    }
    ...
    override public CutsceneTrackItem DuplicateEvent(CutsceneTrackItem ce, CollisionResolutionType resolutionType)
    {
        ...
        CutsceneEvent prevEvent = ce as CutsceneEvent;

        // if the event is duplicated in a different cutscene, this code prevents the event from using the generic events gameobject of
        // event from which is was copied
        if (!GenericEventNames.IsCustomEvent(prevEvent.EventType))
        {
            //GetSelectedCutscene().gameObject.GetComponent((ce as CutsceneEvent).
            //newEvent.SetFunctionTargets(GetSelectedCutscene().transform.GetChild(0).gameObject, (ce as CutsceneEvent).TargetComponent);
            ChangedCutsceneEventType(prevEvent.EventType, newEvent);
            ChangedEventFunction(newEvent, prevEvent.FunctionName, prevEvent.FunctionOverloadIndex);
            prevEvent.CloneParams(newEvent);
        }
    ...
    void ShowFileContextMenu()
    {
        ...
        if (Application.isPlaying)
        {
            contextMenu.AddDisabledItem(new GUIContent("Duplicate Cutscene"));
        }
        else
        {
            contextMenu.AddItem(new GUIContent("Duplicate Cutscene"), false, DuplicateCutsceneMenuItem, GetSelectedCutscene());
        }
        ...
        contextMenu.AddSeparator("Sequencer/");
        contextMenu.AddItem(new GUIContent("Sequencer/Batch Open"), false, FileBatchImport, this);
        contextMenu.AddItem(new GUIContent("Sequencer/Batch Save"), false, FileBatchExport, this);
    ...
    {
        m_SequencerData.m_XmlTitle = EditorUtility.OpenFilePanel("Open XML File", string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)), "xml");
        if (string.IsNullOrEmpty(m_SequencerData.m_XmlTitle))
        {
            return;
        }

        PlayerPrefs.SetString(LastFolderPathKey, Path.GetDirectoryName(m_SequencerData.m_XmlTitle));
    ...
    {
        m_SequencerData.m_XmlTitle = EditorUtility.OpenFilePanel("Open BML File", string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)), "*.txt;*.bml");
        if (string.IsNullOrEmpty(m_SequencerData.m_XmlTitle))
        {
            return;
        }
    ...
    void FileOpenBMLXMLPair(string filePathAndName)
    {
        ...
        GroupManager.SetTrackName(0, "Text Transcript");
        GroupManager.SetTrackName(1, "Viseme Schedule");
        //LoadFile(Path.ChangeExtension(filePathAndName, ".bml"), true); // this load will happen with the xml, not needed here
    }
    ...
    void LoadFile(string filePathAndName, bool setTimes)
    {
        ...
        while (oldName != newName && m_TimeObjectManager.GetTimelineObjectByName(newName) != null)
        {
            newName += '0';
        }
        ChangeCutsceneName(oldName, newName);
    ...
    void FileSaveXMLAs(object data)
    {
        ...
        string filePathAndName = EditorUtility.SaveFilePanelInProject("Save Xml", cutscene.CutsceneName, "xml", "Save Xml",
            string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)));
        if (!string.IsNullOrEmpty(filePathAndName))
        {
            PlayerPrefs.SetString(LastFolderPathKey, Path.GetDirectoryName(filePathAndName));
    ...
    void FileBatchImport(object data)
    {
        string folderName = EditorUtility.OpenFolderPanel("Batch Open Xml", string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)), "");

        if (!string.IsNullOrEmpty(folderName))
        {
            m_SequencerData.m_QueuedFilesToLoad.AddRange(Directory.GetFiles(folderName, "*.xml"));
        }
    }

    void FileBatchExport(object data)
    {
        string folderName = EditorUtility.SaveFolderPanel("Batch Save Xml", string.Format("{0}", PlayerPrefs.GetString(LastFolderPathKey, Application.dataPath)), "");
    ...
    {
        SequencerIO.CreateXml(filePathAndName, cutscene);
    }
    #endregion
    #endregion
}
