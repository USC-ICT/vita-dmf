...
public class RendererEvents : GenericEvents
{
    ...
    {
        protected Renderer GetRenderer(string goName)
        {
            GameObject rendererGo = GameObject.Find(goName);
            Renderer renderer = null;
            if (rendererGo != null)
            {
                renderer = rendererGo.GetComponent<Renderer>();
                if (renderer == null)
                {
                    Debug.LogError("RendererEvent_Base failed. Game object with name " + goName + " doesn't havea a renderer component");
                }
            }
            else
            {
                Debug.LogError("RendererEvent_Base failed. Couldn't find gameobject " + goName);
            }
            return renderer;
        }
    }
    ...
    public class RendererEvent_SetMaterialTexture : RendererEvent_Base
    {
        ...
        public void SetMaterialTexture(string renderer, Texture tex)
        {
            Renderer r = GetRenderer(renderer);
            if (r != null)
            {
        ...
        public override object SaveRewindData(CutsceneEvent ce)
        {
            Renderer renderer = null;//Cast<Renderer>(ce, 0);
            object retVal = null;
            switch (ce.FunctionOverloadIndex)
            {
                case 0:
                    renderer = Cast<Renderer>(ce, 0);
                ...
                case 4:
                    string goName = Param(ce, 0).stringData;
                    renderer = GetRenderer(goName);
                    if (renderer != null)
                    {
                        retVal = renderer.material.mainTexture;
                    }
        ...
        public override void LoadRewindData(CutsceneEvent ce, object rData)
        {
            ...
            switch (ce.FunctionOverloadIndex)
            {
                ...
                case 4:
                    string goName = Param(ce, 0).stringData;
                    SetMaterialTexture(goName, (Texture)rData);
                    break;
            }
        }
        #endregion
