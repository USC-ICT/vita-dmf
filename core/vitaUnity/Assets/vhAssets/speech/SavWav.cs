...
public static class SavWav
{
    ...
    public static bool Save(string filenameAndPath, AudioClip clip)
    {
        return Save(filenameAndPath, ConvertAudioClipToAudioData(clip));
    }

    public static bool Save(string filenameAndPath, AudioData audioData)
    {
        if (!filenameAndPath.ToLower().EndsWith(".wav"))
        {
            filenameAndPath += ".wav";
        }
        ...
        using (var fileStream = CreateEmpty(filenameAndPath))
        {
            ConvertAndWrite(fileStream, audioData.buffer);
            WriteHeader(fileStream, audioData.samples, audioData.frequency, audioData.channels);
        }
    ...
    static void ConvertAndWrite(FileStream fileStream, float[] buffer)
    {
    ...
    static void WriteHeader(FileStream fileStream, int _samples, int _frequency, int _channels)
    {
        var hz = _frequency;
