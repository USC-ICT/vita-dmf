...
public class TimelineInput
{
    ...
    public CutsceneTrackItem GetHorizontalCollider(CutsceneTrackGroupManager groupManager, CutsceneTrackItem trackItem)
    {
        ...
        for (int i = 0; i < trackItems.Count; i++)
        {
            ...
            if (!trackItems[i].Hidden && VHMath.IsRectOverlapping(trackItems[i].GuiPosition, trackItem.GuiPosition))
            {
                return trackItems[i];
            }
        }
        ...
        for (int i = 0; i < editor.m_ItemsToBeAdded.Count; i++)
        {
            ...
            if (!editor.m_ItemsToBeAdded[i].Hidden && !editor.m_ItemsToBeAdded[i].ReadOnly && VHMath.IsRectOverlapping(editor.m_ItemsToBeAdded[i].GuiPosition, trackItem.GuiPosition))
            {
    ...
    public void ResolveHorizontalCollision(CutsceneTrackGroupManager groupManager, CutsceneTrackItem trackItem)
    {
        ...
        for (int i = 0; i < trackItems.Count; i++)
        {
            ...
            if (VHMath.IsRectOverlapping(trackItems[i].GuiPosition, trackItem.GuiPosition))
            {
    ...
    void AttemptSelectTrackGroup(Vector2 relativeMousePos, Event e)
    {
        ...
        if (group != null)
        {
            ...
            // IMPORTANT! if you want to be able to drag track groups, then comment this in.  it was removed because of unity's nesting error
    ...
    void DuplicateEventsContextCallback(object data)
    {
        ...
        //selectedEvents.ForEach(ce => editor.UnSelectTrackItem(ce));
    }
