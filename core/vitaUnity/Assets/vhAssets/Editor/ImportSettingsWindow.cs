...
public class ImportSettingsWindow : EditorWindow {
    ...
    void OnGUI()
    {
        ...
        // SB Motion Generation
        m_CreateSBMotions = GUILayout.Toggle(m_CreateSBMotions, "Create SB Motions");
    ...
    void CreateImportSettingsFile()
    {
        ...
        try
        {
            ...
            sw.WriteLine();
            if (m_CreateSBMotions)
            {
                sw.WriteLine("        if (assetPath.Contains(\"@\") && modelImporter.animationType != ModelImporterAnimationType.Human)");
                sw.WriteLine("             modelImporter.animationType = ModelImporterAnimationType.Legacy;");
            }
            sw.WriteLine("    {0}", "}");
            sw.WriteLine("{0}", "}");
            sw.Close();
