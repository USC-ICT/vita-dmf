...
#pragma exclude_renderers gles
    ...
    */
    ...
    half4 LightingCustomLighting (SurfaceOutput s, half3 lightDir, half3 viewDir, half atten)
    {
        ...
        float3 L = mul(TBN, float4(lightDirFloat, 1)).xyz;
        //lightDir = mul(TBN, lightDirFloat);

        //half4 c = LightingLambert(s, lightDir, atten);            // NOTE: THIS OVERLOAD DISSAPPEARED IN UNITY 5
        half4 c;
        c.rgb = s.Albedo * _LightColor0.rgb * (diff * atten * 2);   // THIS IS WHAT THE FUNCTION USED TO DO. THIS WAS BEING DONE IN THE OLD OVERLOAD, BUT NOW HAS TO BE DONE OUTSIDE
        c.a = s.Alpha;

        float3 colorAsFloat3 = float3(c.r, c.g, c.b);
        ...
        float3 specularColor = specularReflection(NormalSVec, L, viewDir, s.Albedo) * _SpecColor.xyz * atten;
    ...
    void VS(inout appdata_full v, out Input IN)
    {
        ...
        IN.uv_AlbedoD = float2(0, 0); // TODO: this has to be initialized in UNITY 5. Setting it to the default
    }
