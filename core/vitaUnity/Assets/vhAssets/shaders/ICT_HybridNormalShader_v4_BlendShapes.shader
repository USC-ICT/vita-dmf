Shader "VH/ICT_HyrbridNormalShader_v4_BlendShapes" {
...
CGPROGRAM
    ...
    */
    float3 specularReflection(float3 N, float3 L, half3 V, float3 albedoS)
    {
        ...
        float fresnel = pow(min(1, 1 - dot(N, V)), 5.0) * 0.92f + 0.08f;
    ...
    void surf (Input IN, inout SurfaceOutput Out)
    {
        ...
        // blend the specular normals
        float3 n0 = tex2D(_NormalS, IN.uv_Albedo) * 2 - float3(1,1,1);
        float3 n1 = tex2D(_NormalS1, IN.uv_Albedo) * 2 - float3(1,1,1);
        float3 n2 = tex2D(_NormalS2, IN.uv_Albedo) * 2 - float3(1,1,1);
        // correct the length so that blending expressions does not dull the surface (only spatial filtering does)
        float3 l = sqrt(dot(n0, n0) * w0 + dot(n1, n1) * mapWeights.r + dot(n2, n2) * mapWeights.g);
        NormalSVec = normalize(n0 * w0 + n1 * mapWeights.r + n2 * mapWeights.g) * l;
        Out.Normal = NormalSVec;    // [NOTE] putting ANYTHING here tells Unity to work in tangent coordinates, which is nice

        // diffuse normals
        float3 nd = normalize(UnpackNormal(tex2D(_NormalD, IN.uv_Albedo)) * w0 + UnpackNormal(tex2D(_NormalD1, IN.uv_Albedo)) * mapWeights.r + UnpackNormal(tex2D(_NormalD2, IN.uv_Albedo)) * mapWeights.g);
